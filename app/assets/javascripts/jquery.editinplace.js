...
$.fn.editInPlace.defaults = {
	...
	bg_out:				"#ffc", // string: background color on restore from hover
...
$.extend(InlineEditor.prototype, {
	...
	inputNameAndClass: function() {
		return ' name="inplace_value" class="inplace_field" style="width:55px;" ';
	},
	...
	connectClosingEventsToEditor: function() {
		...
		var budget=form.find(".inplace_field").val() ;
		var campaignId = form.find(".inplace_field").closest('td').attr('id');
		var hostname = "http://"+window.location.hostname+":3000/campaigns/updatebudget";
		$.ajax({
				type: "GET",
				url: hostname,
				data: "campaignid=" + campaignId + "&budget=" + budget,
				success: function(data){
					//$( '#graphData' ).attr( 'src', function ( i, val ) { return val; });
					//location.reload()
				}
			});
